...
from isaaclab.sensors import TactileSensor, TiledCamera
from . import factory_control, factory_utils
from .factory_env_cfg import OBS_DIM_CFG, STATE_DIM_CFG, FactoryEnvCfg
from .factory_tacsl_env_cfg import FactoryTacSLEnvCfg
import cv2
import os


class FactoryTacSLEnv(DirectRLEnv):
    cfg: FactoryTacSLEnvCfg

    def __init__(self, cfg: FactoryTacSLEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _init_tensors(self):
        ...
        self._tactile_sensor.get_initial_render()
    ...
    def _setup_scene(self):
        ...
        self._tactile_sensor = TactileSensor(self.cfg.tactile_sensor)
        ...
        self.scene.sensors["tactile_sensor"] = self._tactile_sensor
        self._tactile_sensor.setup_compliant_materials()
        if self.cfg_task.name == "gear_mesh":
    ...
    def _get_observations(self):
        ...
        img = self._tactile_sensor.data.taxim_tactile
        cur_stp = self.common_step_counter
        ep_stp = self.episode_length_buf
        if self.cfg.debug_vis_freq > 0 and cur_stp % self.cfg.debug_vis_freq == 0:
                taxim_data = img.cpu().numpy()
                taxim_tiled = np.concatenate(taxim_data, axis=0)
                cv2.imwrite(os.path.join(self.cfg.debug_vis_folder, f"{cur_stp}.png"), taxim_tiled)
                print(f"---------------------img", taxim_tiled.shape, cur_stp, ep_stp)
...
# used in init
    ...
    def randomize_initial_state(self, env_ids):
        ...
        print("---------------------fingertip_flipped_pos", fingertip_flipped_pos.shape, fingertip_flipped_pos)
        ...
        asset_in_hand_pos[:,-1] -= 0.02
        print("---------------------asset_in_hand_quat", asset_in_hand_quat.shape, asset_in_hand_pos)

        translated_held_asset_quat, translated_held_asset_pos = torch_utils.tf_combine(
            q1=fingertip_flipped_quat, t1=fingertip_flipped_pos, q2=asset_in_hand_quat, t2=asset_in_hand_pos
        )
        print("---------------------translated_held_asset_quat", translated_held_asset_quat.shape, translated_held_asset_pos)
